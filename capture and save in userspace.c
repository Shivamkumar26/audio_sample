...
int main(int argc, char *argv[]) {
    ...
    short *buffer;
    int frames_to_capture, err, duration;
    FILE *wav_file;

    // Check if duration and output file name arguments are provided
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <duration_in_seconds> <output_file.wav>\n", argv[0]);
        return -1;
    }

    // Parse the duration from command line
    ...
    for (int i = 0; i < frames_to_capture / BUFFER_FRAMES; i++) {
        ...
        fwrite(buffer, sizeof(short), BUFFER_FRAMES * CHANNELS, wav_file);
    }
